% FUNCTION: [optsol, output] = scvxPapa3Solver(objFunc, linConstr, x0, y0, 
%                                             options, varargin)
...
%          We assume that either h or r is STRONGLY CONVEX with 
%          paramenter mu_h > 0.
%
% USAGE:
%    Inputs:
...
for iter = 1:options.MaxIters
    ...
    y_hat         = (1 - tau)*y_cur + tau*y_tilde;
    By_hat        = (1 - tau)*By_cur + tau*By_tilde;
    
    % Compute the step-size
    if isfield(objFunc, 'ryGradOper')
        betaL     = beta/((LB_bar + Lipsr*beta)*tau);
    else
    ...
    if isfield(objFunc, 'ryGradOper')
        ...
        y_tilde_next = hyProxOper( y_tilde - betaL*(Btlbd_hats + gradRy_hats), betaL, varargin{:} );
    else
    ...
    if options.isRestart 
        if mod(iter, options.nRestart) == 0% && rel_pfeas < old_rel_pfeas  
            y_tilde_next  = y_next;
