% Example: Solve the Sqrt-LASSO with Elastic net regularizer
...
options.Algorithm    = 'PAPA-SCVX';
time5b               = tic;
[optsol5b, output5b] = constrOptPapaSolver(fzFunc, B, cb, options, y0);
y_opt5b              = optsol5b.y_opt;
fx_val5b             = norm(B*y_opt5b - cb, 2) + 0.5*reg1*norm(y_opt5b, 2)^2 + reg2*norm(y_opt5b, 1);
diff_y5b             = norm(y_opt5b - y_cvx, 2)/nrm_y_cvx;
time5b               = toc(time5b);
...
options.nRestart     = rsFreq2;
time6b               = tic;
[optsol6b, output6b] = constrOptPapaSolver(fzFunc, B, cb, options, y0);
y_opt6b              = optsol6b.y_opt;
fx_val6b             = norm(B*y_opt6b - cb, 2) + 0.5*reg1*norm(y_opt6b, 2)^2 + reg2*norm(y_opt6b, 1);
diff_y6b             = norm(y_opt6b - y_cvx, 2)/nrm_y_cvx;
time6b               = toc(time6b);
...
options.Algorithm    = 'PAPA-SCVX';
time5c               = tic;
[optsol5c, output5c] = constrOptPapaSolver(fzFunc, B, cb, options, y0);
y_opt5c              = optsol5c.y_opt;
fx_val5c             = norm(B*y_opt5c - cb, 2) + 0.5*reg1*norm(y_opt5c, 2)^2 + reg2*norm(y_opt5c, 1);
diff_y5c             = norm(y_opt5c - y_cvx, 2)/nrm_y_cvx;
time5c               = toc(time5c);
...
options.nRestart     = rsFreq2;
time6c               = tic;
[optsol6c, output6c] = constrOptPapaSolver(fzFunc, B, cb, options, y0);
y_opt6c              = optsol6c.y_opt;
fx_val6c             = norm(B*y_opt6c - cb, 2) + 0.5*reg1*norm(y_opt6c, 2)^2 + reg2*norm(y_opt6c, 1);
diff_y6c             = norm(y_opt6c - y_cvx, 2)/nrm_y_cvx;
time6c               = toc(time6c);
...
%% Plot the outputs.
fx_min  = min([fx_val1; fx_val2; fx_val3; fx_val4; fx_val5; fx_val6; fx_val7; fx_val8; fx_val9; fx_cvx; fx_val5b; fx_val5c; fx_val6b; fx_val6c]);
YMatrix{run} = [abs(output1.hist.usedef  - fx_min)/max(1, abs(fx_min)), ...
                ...
                abs(output5b.hist.usedef - fx_min)/max(1, abs(fx_min)), ...
                abs(output5c.hist.usedef - fx_min)/max(1, abs(fx_min)), ...
                abs(output6.hist.usedef  - fx_min)/max(1, abs(fx_min)), ...
                abs(output6b.hist.usedef - fx_min)/max(1, abs(fx_min)), ...
                abs(output6c.hist.usedef - fx_min)/max(1, abs(fx_min)), ...
...
%% Generate a figure
createFigure1c(YMatrix{1}, YMatrix{2});
