% FUNCTION: [optsol, output] = nscvxLinearizedPapaSolver(objFunc, linConstr, 
%                                                x0, y0, options, varargin)
% PURPOSE: An implementation of the PAPA algorithm for solving 
%          constrained convex optimization problems of the form:
%
%                    min_x g(x) + h(y) s.t. A*x + B*y in Kc,
%
%          where g and h are convex, and Kc is a given convex set.
%          Here, we do not assume h or g to be strongly convex.
%
% USAGE:
%    Inputs:
...
% Evaluate the l2-norm of A'*A and B'*B.
LA_bar      = linConstr.LA_bar;
...
for iter = 1:options.MaxIters
    ...
    if options.isFxEval
        ...
        dl_val = dlStarFunc(Ax_cur + By_cur, varargin{:});
    end
    ...
    betaL        = beta/(LA_bar + LB_bar);
    
    % Compute the dual vector lbd_hat_star.
    lbd_bar_star = dlStarProx(lbd_dot + 1/beta * (Ax_hat + By_hat), 1/beta);
    
    % Compute the proximal operator of g and compute Ax_next.
    Atlbd_hats   = AToper(lbd_bar_star);
    x_next       = gxProxOper( x_hat - betaL*Atlbd_hats, betaL, varargin{:} );
    Ax_next      = Aoper(x_next);
    
    % Compute the dual vector lbd_hat_star2.
    lbd_hat_star = dlStarProx(lbd_dot + 1/beta * (Ax_next + By_hat), 1/beta);
    ...
    y_hat      = y_next  + ((tau_next * (1 - tau))/tau) * (y_next  - y_cur);
